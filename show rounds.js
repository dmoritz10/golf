async function btnShowRoundsHtml() {
  ...
  for (var j = rnds.length - 1; j > endRow - 1; j--) {
    ...
    if (srMadeTarget && !madeTargetScore(targetScore, roundObj.finalScore)) {
      continue;
    }
  ...
  for (var j = 0; j < rounds.length; j++) {
    ...
    var cntr = 0;
    sc.scores.map((val, idx) => {
      if (val) {
        if (val.score - val.putts <= val.par - 2) cntr++;
      }
    });
    ...
    ele.find("#srHcpDiff")[0].innerHTML = escCorrections
      ? hcpDiff + "<sup>" + escCorrections + "</sup>"
      : hcpDiff;
    ...
    ele
      .find("#srFetchRound")[0]
      .setAttribute("onclick", "showRoundDetail(" + roundObj.rowIdx + ", 'ShowRounds')");
  ...
  $("#srContainer > div").click(function (e) {
...
async function btnSRSelectHtml(e) {
  ...
  var srSelectedCourseVal =
    $("#srSelectCourse").val() > -1 ? $("#srSelectCourse option:selected").text() : false;
  var srSelectedSortBy = $("#selectRoundsSortBy option:selected").text();
  var srSelectedDateRngVal =
    document.getElementById("selectRoundsDateRng").selectedIndex > 0
      ? $("#selectRoundsDateRng option:selected").text()
      : false;
...
*/
  ...
  var getAllRoundsReq =
    "https://api.swingbyswing.com/v2/players/" +
    myPlayerId +
    "/rounds?from=1&limit=" +
    "5" +
    "&access_token=" +
...
function getSxsRounds(rounds) {
  rounds.forEach((val, idx, arr) => {
    ...
    var getRoundReq =
      "https://api.swingbyswing.com/v2/rounds/" +
      val.id +
      "?includes=scorecards,course&access_token=" +
      clientId;
